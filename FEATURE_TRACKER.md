...
2. **🩹 Wound measurements are all `0.0 × 0.0 cm` — result-screen crash FIXED, root cause still open.** The 3 real rows carry Model 2/3 outputs (`tissueType='Callus'`, `infection='Present'`) but `length=width=0`. In `ai_service.dart`, `length`/`width` are only set inside `if (_model1Loaded)`; the CLIP-backbone heads (Models 2/3) loaded but **Model 1 (segmentation) did not**, so measurements stayed 0 and were saved silently. **Done this session:** the result screen no longer crashes on 0-area data (see the chart fix in "What happened"). **Still open:** (a) get device model-load logs to see *why* `model1.tflite` failed to load while the others succeeded; (b) consider blocking the save (or marking "not measured") when `isFromModel` but `length*width == 0`, so 0×0 wounds never pollute the trend baseline. The healing trend still can't plot meaningfully for real users until capture writes real cm — but it no longer errors.
