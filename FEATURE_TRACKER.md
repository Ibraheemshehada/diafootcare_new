...
2. **🩹 Wound measurements are `0.0 × 0.0 cm` — most likely because the test photos are NOT wounds (emulator virtual scene), not a model-load failure.** ⚠️ *Correction to an earlier note in this file:* I first assumed "Model 1 (segmentation) failed to load." That was probably wrong. `ai_service.dart` loads all three models in one lazy `init()` — Model 1 (`model1_wound_fp16.tflite`, a valid 12.4 MB file) **first**, then the 175 MB CLIP backbone. CLIP + heads demonstrably loaded (the rows carry `tissueType='Callus'`, `infection='Present'`), so there is no plausible reason the smaller Model 1 loaded right before it would fail. And `_runSegmentation` returns `(0,0,0)` whenever it finds **no wound** in the image (empty probability map → line 269; no connected component → line 282). The 3 rows were captured on the **emulator, whose camera is a synthetic room scene — no wound → empty mask → 0×0.** **To confirm on a real device:** capture an actual wound and check the result, or pull logcat during analysis and look for `✅ Model 1: L=.. W=..` (works) vs `⚠️ Failed to load Model 1` (a real load failure). **Done this session:** the result screen no longer crashes on 0-area data (see the chart fix). **Optional hardening:** when `isFromModel` but `length*width == 0`, consider telling the user "no wound detected — reframe and retry" instead of saving a 0×0 row.
3. **Consider raising body text 12sp → 14sp** for elderly diabetics (minimum is 12sp everywhere now).
4. **`dart format` divergence.** The repo is *not* dart-formatted (89 of 117 files would change). I formatted the 13 files touched by the fixed-height refactor, so their diffs are larger than the semantic change. Either format the whole repo once, or don't format at all — pick one.
5. **Voice assistant polish (optional).** `SpeakButton` now sits on the education article, the SUS declaration, and the self-care rotating tip. Consider a global on/off toggle in Profile.
